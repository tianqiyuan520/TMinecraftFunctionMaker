...
class Parser(Storage_editor):
    ...
    def __init__(self,content,*args,**kwargs) -> None:
        ...
        # data为记录数据,is_return控制当前函数是否返回,is_end控制该函数是否终止,is_break控制循环是否终止,is_continue是否跳过后续的指令,return 记录调用的函数的返回值 , exp_operation 表达式运算的过程值(栈) is_global 判断该变量是否为全局变量 functions记录函数参数列表,函数调用接口,当前函数 BoolOpTime 条件判断 累计次数 Subscript为记录切片结果 condition_time记录该函数逻辑运算时深度遍历的编号（属于临时变量） "boolOPS"记录逻辑运算的结果 boolResult 记录实际逻辑运算结果（用于判断and or）elif_time 记录 elif次数(临时数据) while_time记录 while次数 for_time 记录 for 次数 for_list 记录for迭代的列表([{"value":xx},{..},...]) "call_list" 调用函数的参数列表(  [{"value":"","id":"",is_constant:True}   ]   ,若有id则为kwarg ) , class_list 记录 自定义类的函数与数据 "record_condition" 统计逻辑次数
        #  类型:  BinOp Constant Name BoolOp Compare UnaryOp Subscript list tuple Call Attribute
        self.main_tree:list[dict] = [{"data":[],"is_break":0,"is_continue":0,"return":[],"is_return":0,"is_end":0,"exp_operation":[],"functions":[],"BoolOpTime":0,"condition_time":0,"elif_time":0,"while_time":0,"for_time":0,"call_list":[],"list_handler":[],"class_list":[],"record_condition":[]}]
        #初始化计分板
        self.write_file('load',f'scoreboard objectives add {scoreboard_objective} dummy\n',**kwargs)
        #初始化计分板2 （记录if逻辑中的 临时记分板）
        self.write_file('load',f'scoreboard objectives add {scoreboard_objective}2 dummy\n',**kwargs)
...
## 工具方法
    ...
    def py_append_tree(self):
        '''新建堆栈值'''
        self.main_tree.append({"data":[],"is_break":0,"is_continue":0,"return":[],"type":"","is_return":0,"is_end":0,"exp_operation":[],"functions":[],"BoolOpTime":0,"condition_time":0,"elif_time":0,"while_time":0,"for_time":0,"call_list":[],"list_handler":[],"class_list":[],"record_condition":[]})
        return self
    # 获取两数运算结果
        ...
        if isinstance(tree.value,ast.BinOp):
            ...
            for i in tree.targets:
                if isinstance(i,ast.Name):
                    ...
                    self.mcf_modify_value_by_from(f'storage {defualt_STORAGE} main_tree[-1].data[{{"id":"{i.id}"}}].value','set',f'storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":0}}].value[-1]',func,**kwargs)
                elif isinstance(i,ast.Attribute):
                    ClassC = ClassCall(self.main_tree,**kwargs)
                    StoragePath = ClassC.class_var_to_str(i,func,**kwargs)
                    self.mcf_modify_value_by_from(f'{StoragePath}','set',f'storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":0}}].value[-1]',func,**kwargs)
    ...
    def If(self,tree:ast.If,condition_time:0,func:str,mode:True,*args,**kwargs):
        ...
        if isinstance(tree.test,ast.BoolOp):
            ...
            if mode:
                ...
                self.write_file(func,f'execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.pass {scoreboard_objective} run data get storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value[-1]\n',**kwargs)
            else:
    ...
    def BoolOp(self,tree:ast.BoolOp,condition_time:0,func:str,mode:bool = True,*args,**kwargs):
        ...
        for item in tree.values:
            ...
            if isinstance(item,ast.BoolOp):
                ...
                self.write_file(func,f'execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.temp {scoreboard_objective} run data get storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value[-1]\n',**kwargs)
                kwargs = kwargs_
                self.BoolOp_record(tree,condition_time,self.main_tree[-1]["condition_time"],func,**kwargs)
...
#             self.write_file(func,f'''
# ##和
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count1 {scoreboard_objective} run data get storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count2 {scoreboard_objective} run data modify storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value[] set value 0
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run scoreboard players operation #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count1 {scoreboard_objective} -= #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count2 {scoreboard_objective}
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute if score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count1 {scoreboard_objective} matches 1.. run data modify storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time2}}}].value append value 0
# ''',**kwargs)
...
# f'''##或
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count1 {scoreboard_objective} run data get storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count2 {scoreboard_objective} run data modify storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value[] set value 1
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run scoreboard players operation #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count1 {scoreboard_objective} -= #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count2 {scoreboard_objective}
# execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute if score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.count1 {scoreboard_objective} matches 1.. run data modify storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time2}}}].value append value 1
# ''',**kwargs)
...
# 记录 运算后的布尔值
    def BoolOp_record(self,tree:ast,condition_time:int,index:int,func:str,*args,**kwargs):
        ...
        # self.mcf_change_value_by_scoreboard(f'{defualt_STORAGE} main_tree[-1].boolResult[{{"id":{condition_time}}}].value[-1]',f'#{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.{index} {scoreboard_objective}','int',1,func,**kwargs)
...
# while
    def While(self,tree:ast.While,func:str,*args,**kwargs):
        ...
        if isinstance(tree.test,ast.Name):
            ...
            self.write_file(func,f'execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute store result score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.pass {scoreboard_objective} run data get storage {defualt_STORAGE} main_tree[-1].boolResult[{{"id":0}}].value[-1]\n',**kwargs)
            self.main_tree[-1]['condition_time'] = 0
        elif isinstance(tree.test,ast.Subscript):
            self.mcf_call_function(f'{func}/condition_{(self.main_tree[-1]["BoolOpTime"])}/if/{self.main_tree[-1]["condition_time"]}',func,**kwargs)
            kwargs['f2'] = f'{self.main_tree[-1]["condition_time"]}'
...
# for
    def For(self,tree:ast.For,func:str,*args,**kwargs):
        ...
        self.mcf_call_function(f'{func}/for_{self.main_tree[-1]["for_time"]}/main',func,False,f'execute unless score #{defualt_STORAGE}.stack.end {scoreboard_objective} matches 1 run execute if score #{defualt_NAME}.sys.c.{(self.main_tree[-1]["BoolOpTime"])}.pass {scoreboard_objective} matches 1 run ',**kwargs)
