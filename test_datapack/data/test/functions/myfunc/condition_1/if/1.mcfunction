...
execute unless score #test:system.stack.end input matches 1 run data modify storage test:system main_tree[-1].boolOPS[-1].value set from storage test:system main_tree[-1].data[{"id":"i"}].value
execute unless score #test:system.stack.end input matches 1 run data modify storage test:system main_tree[-1].boolOPS append value {"value": 10}
